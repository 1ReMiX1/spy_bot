from telegram.ext import ApplicationBuilder, CommandHandler, CallbackQueryHandler, ContextTypes, MessageHandler, PreCheckoutQueryHandler, ChatMemberHandler, filters
...
# Автозагрузка переменных из файла .env (для локального запуска).
# На хостинге переменные задаются в панели — этот блок просто ничего не делает.
try:
...
DB_PATH = 'spy_bot.db'
# URL мини-приложения (Web App). Должен быть HTTPS.
# Для локального теста поднимите HTTPS-туннель (cloudflared/ngrok) на папку miniapp
# и положите ссылку в переменную окружения WEBAPP_URL.
WEBAPP_URL = os.getenv('WEBAPP_URL', '').rstrip('/')
# Группа, подписку на которую проверяем (публичный @username или числовой chat_id).
# Бот должен состоять в этой группе (лучше админом), чтобы видеть участников.
GROUP_CHAT = os.getenv('GROUP_CHAT', '@ggamedag')
...
THEMES = {
    ...
    "🎬 Фильмы": [
        ...
        "Чужой", "Терминатор", "Робокоп", "Пятый эле��ент", "Касабланка",
    ],
    "🦁 Животные": [
        "Лев", "Тигр", "Медведь", "Волк", "Лиса", "Заяц", "Олень",
        "��рел", "Акула", "Крокодил", "Слон", "Жираф", "Зебра", "Панда",
...
TOD_QUESTIONS = {
    "normal": {
        ...
        "dare": [
            ...
            "Сделай компл��мент самому себе вслух.",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Старт: одна кнопка, которая открывает мини-приложение."""
    try:
        if update.effective_user:
            await _refresh_membership(context, update.effective_user.id)
    except Exception:
        pass
    if not WEBAPP_URL:
        await update.message.reply_text(
            "⚠️ Мини-приложение не настроено.\n"
            "Задайте WEBAPP_URL с HTTPS-ссылкой на мини-приложение."
        )
        return
    url = WEBAPP_URL + "/?v=7"  # метка версии — чтобы Telegram не брал страницу из кэша
    keyboard = [
        [InlineKeyboardButton("🎮 Открыть игры", web_app=WebAppInfo(url=url))],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = "👋 Добро пожаловать в GameDAG!\n\nНажми кнопку — выбери игру прямо в приложении:"
...
async def tod_choose_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("✅ Выполнено / Отв��тил", callback_data=f"tod_done_{code}")],
        ...
        try:
            await context.bot.send_message(
                ...
                text=(f"🐊 КРОКОДИЛ НАЧАЛСЯ!\n\n🎯 Сложность: {lobby.difficulty}\n"
                      ...
                      f"Когда придёт твой черёд объяснять — ��ы получишь слово.\n"
...
async def vote_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if parts[1] == "single":
        ...
        if votee_id not in lobby.players:
            await query.edit_message_text("❌ Игрок не найд��н.")
            return
...
async def join_mafia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lobby.single_device:
        await update.message.reply_text("❌ Это лобби для игры с одного у��тройства")
        return
    if lobby.started:
        await update.message.reply_text("❌ Игра уже началась.")
        return
...
# ============= TELEGRAM STARS / ЗВЁЗДОЧКИ =============
FIREBASE_DB_URL = os.getenv('FIREBASE_DB_URL', 'https://bot1-bc2d2-default-rtdb.firebaseio.com').rstrip('/')

STAR_PACKS = {
    'p1000':  {'coins': 1000,  'stars': 49,  'title': '1 000 звёздочек'},
    'p2500':  {'coins': 2500,  'stars': 99,  'title': '2 500 звёздочек'},
    'p6000':  {'coins': 6000,  'stars': 199, 'title': '6 000 звёздочек'},
    'p15000': {'coins': 15000, 'stars': 449, 'title': '15 000 звёздочек'},
}


def _fb_url(path):
    return FIREBASE_DB_URL + '/' + path.lstrip('/') + '.json'
...
def fb_get(path):
    try:
        ...
        logger.warning(f'fb_get {path}: {e}')
        return None
...
def fb_patch(path, data):
    try:
        ...
        req = urllib.request.Request(_fb_url(path), data=body, method='PATCH',
                                     headers={'Content-Type': 'application/json'})
        with urllib.request.urlopen(req, timeout=10) as r:
            return json.loads(r.read().decode('utf-8'))
    except Exception as e:
        logger.warning(f'fb_patch {path}: {e}')
        return None


def credit_coins(user_id, coins, name=''):
    path = 'users/u' + str(user_id)
    cur = fb_get(path) or {}
    new_balance = int(cur.get('coins', 0) or 0) + int(coins)
    upd = {'coins': new_balance, 'purchased': True}
...
def _is_member_status(st, is_member_flag=False):
    return st in ('member', 'administrator', 'creator', 'owner') or (st == 'restricted' and is_member_flag)


async def _refresh_membership(context, user_id):
    """Проверяет, состоит ли пользователь в группе, и пишет флаг в Firebase."""
    try:
        m = await context.bot.get_chat_member(GROUP_CHAT, user_id)
        st = getattr(m, 'status', '')
        is_member = _is_member_status(st, getattr(m, 'is_member', False))
        fb_patch('members', {'u' + str(user_id): bool(is_member)})
        return is_member
    except Exception as e:
...
async def chat_member_update(update, context):
    """Срабатывает, когда пользователь вступает в группу или выходит из неё."""
    ...
    try:
        ...
        target = GROUP_CHAT.lstrip('@').lower()
        if uname and uname != target:
            return
        new = cm.new_chat_member
        st = getattr(new, 'status', '')
        is_member = _is_member_status(st, getattr(new, 'is_member', False))
        uid = new.user.id
        fb_patch('members', {'u' + str(uid): bool(is_member)})
    except Exception as e:
        logger.warning(f'chat_member_update: {e}')


async def on_startup(application):
    """При старте: сохраняем username бота и создаём ссылки на оплату звёздами."""
    try:
        me = await application.bot.get_me()
        if me and me.username:
            fb_patch('config/bot', {'username': me.username})
    except Exception as e:
        logger.warning(f'on_startup get_me: {e}')
    for pack_id, p in STAR_PACKS.items():
        try:
            link = await application.bot.create_invoice_link(
                title=p['title'],
                description=f"Пополнение баланса: +{p['coins']} звёздочек",
                payload='pack:' + pack_id,
                currency='XTR',
                prices=[LabeledPrice(p['title'], p['stars'])],
            )
            fb_patch('config/starPacks/' + pack_id,
                     {'coins': p['coins'], 'stars': p['stars'], 'title': p['title'], 'link': link})
            logger.info(f'invoice link ready: {pack_id}')
        except Exception as e:
            logger.warning(f'create_invoice_link {pack_id}: {e}')
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning(f'precheckout: {e}')
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith('pack:'):
        ...
        p = STAR_PACKS.get(pack_id)
        if p:
            new_balance = credit_coins(user.id, p['coins'], name)
            await update.message.reply_text(
                f"✅ Оплата прошла! Начислено {p['coins']} звёздочек.\n"
                f"Баланс: {new_balance} \U0001F31F\n"
                f"Открой приложение, чтобы потратить их в магазине."
            )
            return
    await update.message.reply_text("✅ Оплата получена, спасибо!")
...
def main():
    try:
        ...
        # Платежи Telegram Stars (звёздочки)
        app.add_handler(PreCheckoutQueryHandler(precheckout_handler))
        app.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, successful_payment_handler))

        # Проверка подписки на группу (вступление/выход)
