...
def _webapp_btn(label, game):
    """Кнопка, открывающая мини-приложение на нужной игре."""
    url = WEBAPP_URL + ("?game=" + game if game else "")
    return InlineKeyboardButton(label, web_app=WebAppInfo(url=url))


async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Стартовое меню — игры открываются в мини-приложении."""
    if not WEBAPP_URL:
        ...
        return
    keyboard = [
        [_webapp_btn("🎮 Открыть игры", "")],
        [_webapp_btn("🕵️ ШПИОН", "spy")],
        [_webapp_btn("🔪 МАФИЯ", "mafia")],
        [_webapp_btn("🐊 КРОКОДИЛ", "crocodile")],
        [_webapp_btn("🎭 ПРАВДА ИЛИ ДЕЙСТВИЕ", "tod")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = "👋 Добро пожаловать в GameDAG!\n\nВыбери игру — она откроется в мини-приложении:"
